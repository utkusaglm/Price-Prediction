...
def train_columns(df):
    """ train_columns """
    global SYMBOLS
    train_ready = {}
    for symbol in SYMBOLS: 
        train_columns = [list(c)[0] for c in df.columns.values if symbol in str(c)]
        train_columns.extend([f'{c_name}_Shifted_Log_Return' for c_name in SYMBOLS if c_name != symbol] )
...
if __name__ == '__main__':
    ...
    for symbol in SYMBOLS:
        print('Random Forest')
        random_forest(df,t_columns[symbol],symbol)
        print('---------------------------------')
        print('Logistic Reg')
        logistic_reg(df,t_columns[symbol],symbol)
        print('---------------------------------')
